...
impl XmlElementDef {
    ...
    fn from_fields<T, P>(fields: &FieldsNamed, obj_meta: Option<&MetaInfo>, parse: P) -> Vec<T>
        where P: Fn(Ident, Option<&MetaInfo>, Option<&MetaInfo>, XmlExtension) -> Option<T> {

        fields.named.iter().filter_map(|field| {
            parse(field.ident.clone()?, obj_meta, MetaInfo::from_name(&field.attrs, MetaName::XML).as_ref(), XmlExtension::from_field(field))
        }).collect()
    }

    fn from_fields_first<T, P>(fields: &FieldsNamed, obj_meta: Option<&MetaInfo>, parse: P) -> Option<T>
        where P: Fn(Ident, Option<&MetaInfo>, Option<&MetaInfo>, XmlExtension) -> Option<T>, T: Clone {
