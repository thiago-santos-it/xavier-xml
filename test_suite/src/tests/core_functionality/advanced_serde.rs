...
#[derive(XmlSerializable, XmlDeserializable, Debug, PartialEq)]
struct XmlObjectComplex {
    pub id: u32,
    pub name: String,
    pub attributes: Vec<KeyValue>,
    pub children: Vec<XmlObjectComplex>,
}
...
#[test]
fn advanced_inner() -> Result<(), PError> {

    let test_data = XmlObjectWithInner {
        id: 123,
    ...
    let parsed: XmlObjectWithInner = from_xml(&xml)?;
    assert_eq!(test_data, parsed);

    // Verify that it contains all necessary tags
    assert!(xml.contains("<XmlObjectWithInner>"));
    assert!(xml.contains("</XmlObjectWithInner>"));
...
#[test]
fn advanced_complex() -> Result<(), PError> {

    let test_data = XmlObjectComplex {
        id: 1,
        name: "Root".to_string(),
        attributes: vec![
    ...
    assert!(xml.contains("<XmlObjectComplex>"));
    assert!(xml.contains("</XmlObjectComplex>"));
    ...
    let parsed: XmlObjectComplex = from_xml(&xml)?;
...
#[test]
fn advanced_round_trip() -> Result<(), PError> {

    let test_data = XmlObjectFormatted {
        id: 456,
    ...
    let parsed1: XmlObjectFormatted = from_xml(&xml1)?;

    // Second serialization of parsed result
    let xml2 = from_obj(&parsed1);
    let parsed2: XmlObjectFormatted = from_xml(&xml2)?;

    // Third serialization
    let xml3 = from_obj(&parsed2);
    let parsed3: XmlObjectFormatted = from_xml(&xml3)?;
...
#[test]
fn advanced_encoding() -> Result<(), PError> {
    ...
    // Verify that it can be parsed back
    let _parsed: XmlObjectWithInner = from_xml(&xml)?;

    Ok(())
}

#[test]
fn advanced_empty_values() -> Result<(), PError> {
...
#[test]
fn advanced_numeric_precision() -> Result<(), PError> {
...
#[test]
fn advanced_boolean_values() -> Result<(), PError> {
    ...
    // Verify that boolean values were advancedd correctly
...
#[test]
fn advanced_collection() -> Result<(), PError> {
    let test_data = XmlObjectFormatted {
        id: 999,
    ...
    // Verify that it can be parsed back
    let parsed: XmlObjectFormatted = from_xml(&xml)?;
    assert_eq!(test_data, parsed);
...
fn advanced_malformed() {
    ...
    let result = from_xml::<XmlObjectWithInner>(malformed_xml);
    assert!(result.is_err());
}

#[test]
fn advanced_white_space() -> Result<(), PError> {
    let test_data = XmlObjectWithInner {
        id: 123,
