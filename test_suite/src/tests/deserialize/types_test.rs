...
#[test]
fn deserialize_special_characters() -> Result<(), PError> {
    let xml = r#"
    <XMLObject>
        <type_i8>42</type_i8>
        <type_i16>-100</type_i16>
        ...
        <type_f64>3.14159265358979323846264338327950288</type_f64>
        <type_bool>true</type_bool>
        <type_char>é</type_char>
    </XMLObject>"#;
    let obj: XMLObject = from_xml(&xml)?;
    assert_eq!(obj.type_string, "& < > \" '");
    assert_eq!(obj.type_char, 'é');

    Ok(())
}

#[test]
fn deserialize_cdata() -> Result<(), PError> {
    let xml = r#"
    <XMLObject>
        <type_i8>42</type_i8>
        <type_i16>-100</type_i16>
        ...
        <type_isize>-42</type_isize>
        <type_usize>42</type_usize>
        <type_string><![CDATA[<script>alert('Hello')</script>]]></type_string>
        <type_f32>3.14</type_f32>
        <type_f64>3.14159265358979323846264338327950288</type_f64>
        <type_bool>true</type_bool>
        <type_char>A</type_char>
    ...
    let obj: XMLObject = from_xml(&xml)?;
    assert_eq!(obj.type_string, "<script>alert('Hello')</script>");
